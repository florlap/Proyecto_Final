...
import { getAllUsers ,userActDes} from './usersActions'
...
export default function UsersAdmin() {
    ...
    function dataForm(data, mode) {
        ...
        //     studentId: data?.idUser?.map(student => student.idUser)
        // }
        // if (mode === "new") {
    ...
    return (
        <div>
            ...
            <Typography align={"center"} sx={{ mt:2}} gutterBottom variant="h4" >Administración de Usuarios</Typography>
                <Box sx={{ ml:2}}>
                    <Button variant="outlined" onClick={() => handleOnclickNewEdit({}, "new", "Crear de Alumno")}>Crear Usuario</Button>
                </Box>
                <Box sx={{ m:2}}>
                    <MUIDataTable
            ...
            <Grid Container >
                {/* <DialogContainer open={open}  ><NotificationForm notification={userSelected} mode={mode} handleClose={handleClose} title={title} dataForm={dataForm} /></DialogContainer> */}
            </Grid>
        </div>
    )
}
